...
ツールバーの拡張機能アイコンを押すと、5種類のパレットから選べます。選んだ瞬間に、開いている AtCoder のタブにもリロードなしで反映されます。
...
「🎲 おまかせ」ボタンを押すと、5パレットからランダムに1つ選んでその場で適用します。

### カスタム配色

メイン・アクセント・サブの3色を自分で選んで、独自のパレットを作れます。「このカスタム配色を使う」を押すと適用されます(background/text など残りの色は既定のライトトーンのまま、選んだ3色から自動で濃淡違いのバリエーションを計算します)。

### コードブロックだけ配色を固定

「コードブロック」欄で「ライト」「ダーク」を選ぶと、ページ全体のパレットとは関係なく、コード表示部分(提出コード欄・シンタックスハイライトなど)だけ配色を固定できます。「自動」に戻すとページのパレットに合わせます。サンプル入出力欄は、読みやすさのため常にページのパレットに従います。

選択内容はすべて `chrome.storage.local` に保存され、次回以降も引き継がれます。
