...
AtCoder / AOJ / yukicoder の問題ページにサンプル入出力を ZIP でダウンロードするボタンを追加し、AtCoder の問題・個別解説ページに HTML コピーボタンを追加するブラウザ拡張機能です。

## 機能

- `https://atcoder.jp/contests/*/tasks/*` の問題ページで動作します。
- `https://atcoder.jp/contests/*/tasks_print` の印刷用ページでは、AtCoder HTML コピーボタンで全問題の HTML をまとめてコピーします。
...
- AtCoder の個別解説ページでは `解説HTMLをコピー` ボタンを表示し、その解説本文だけをコピーします。
- 複数の解説本文が展開されているページでは、各解説に `解説HTMLをコピー` ボタンを表示し、クリックした1件だけをコピーします。
- 解説リンク一覧だけのページでは `解説HTMLをコピー` ボタンを表示しません。
...
AtCoder の問題ページでは `問題文をコピー`、個別解説ページでは `解説HTMLをコピー` ボタンを押します。
